					...
					<div class="col-xl-8">
						<table class="table table-striped" style="width:100%">
							...
							<tbody v-for="order in orders">
								...
								<tr>
									<td><button v-if="currentUser.role && (((order.status === 'PROCESSING' || order.status === 'PREPARATION') && currentUser.role === 'MANAGER') || ((order.status === 'AWAITING_DELIVERY' || order.status === 'IN_TRANSPORT') && currentUser.role === 'DELIVERY'))" v-on:click="upgradeOrderStatus(order)">Upgrade</td>
								</tr>
							</tbody>
						</table>
					</div>
